...
SOURCE_DOCX = ROOT / "毕业论文_最终稿_引文闭环版.docx"
SOURCE_MD = ROOT / "毕业论文_正文提取版_引文闭环终稿源.md"
OUTPUT_DOCX = ROOT / "毕业论文_最终稿_引文闭环重构版.docx"
...
def markdown_to_blocks(text: str) -> list[ET.Element]:
    ...
    while i < len(lines):
        ...
        if not in_body:
            if stripped.startswith("## 第一章"):
                in_body = True
            else:
                i += 1
                continue

        if stripped.startswith("## 中文摘要") or stripped.startswith("## Abstract"):
            i += 1
            continue
        ...
        if stripped.startswith("图 ") or stripped.startswith("表 "):
            blocks.append(parse_fragment(p_center(stripped, size=24, bold=False)))
            i += 1
            continue
