...
$output = Join-Path $root '毕业论文_最终稿_模板优先修复版.docx'
...
function InsertParagraphBeforeIndex([int]$index, [string]$text, [string]$kind) {
  ...
  $range.InsertParagraphBefore() | Out-Null
...
function InsertParagraphsAfterMarker([string]$marker, [string]$text) {
  $index = (Find-ParagraphIndex $doc $marker) + 1
  $parts = @(($text -split "(`r`n|`n){2,}") | Where-Object { $_.Trim() })
  for ($i = $parts.Count - 1; $i -ge 0; $i--) {
    InsertParagraphBeforeIndex $index $parts[$i].Trim() 'body'
  }
}

function InsertCaptionBlock([string]$anchor, [string[]]$captions, [string[]]$paragraphs) {
  $index = (Find-ParagraphIndex $doc $anchor) + 1
  $items = @()
  foreach ($caption in $captions) {
    $items += [pscustomobject]@{ Kind = 'caption'; Text = $caption }
  }
  foreach ($paragraph in $paragraphs) {
    $items += [pscustomobject]@{ Kind = 'body'; Text = $paragraph }
  }
  for ($i = $items.Count - 1; $i -ge 0; $i--) {
    InsertParagraphBeforeIndex $index $items[$i].Text $items[$i].Kind
  }
}
...
function NormalizeHeadingStyles() {
  for ($i = 1; $i -le $doc.Paragraphs.Count; $i++) {
    ...
    } elseif ($text -in @('摘    要','摘 要','ABSTRACT','目  录','目录','结论','参考文献','致谢')) {
      $para.Range.Style = $doc.Styles.Item('标题 1')
      $para.Range.ParagraphFormat.Alignment = 1
...
function SetPageBreakBefore([string]$text) {
  $index = Find-ParagraphIndex $doc $text
  $doc.Paragraphs.Item($index).Format.PageBreakBefore = $true
}
...
try {
  ...
  if ((Get-ParaText $doc.Paragraphs.Item(2)) -eq '/') {
    $doc.Paragraphs.Item(2).Range.Delete()
  }

  InsertCaptionBlock '2.1 业务场景分析' @('图 2-1 系统业务流程图') @(
  ...
  InsertCaptionBlock '5.3 性能测试与兼容性测试' @('图 5-1 性能测试记录图', '图 5-2 兼容性测试结果图') @(
  ...
  foreach ($item in $insertions) {
    InsertParagraphsAfterMarker $item.Marker $item.Text
  }

  NormalizeHeadingStyles

  SetPageBreakBefore '摘    要'
  SetPageBreakBefore 'ABSTRACT'
  ...
  foreach ($item in $captions) {
    ...
    $captionPara.Range.InsertParagraphBefore() | Out-Null
    $imgPara = $doc.Paragraphs.Item($item.Index)
    $imgRange = $imgPara.Range.Duplicate
    $imgRange.Collapse(0)
    $shape = $doc.InlineShapes.AddPicture($item.Path, $false, $true, $imgRange)
    ...
    ApplyCaptionStyle $captionPara.Range
  }
  ...
  if ($src -ne $null) { try { $src.Close() } catch {} }
